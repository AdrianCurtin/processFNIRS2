    ...
    for d = 1:nDyads
        ...
        if length(members) ~= groupSize
            warning('exploreFNIRS:hyperscanning:pairSubjects', ...
                ['Group "%s" has %d members (expected %d). Skipping.\n', ...
                 '  Fix: ensure each member has .info.%s = ''%s'', ', ...
                 'or pass ''GroupSize'', %d to match actual group size.'], ...
                dID, length(members), groupSize, dyadField, dID, length(members));
            continue;
        end
