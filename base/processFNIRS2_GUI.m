...
if(PF2.mergedProbe) %All channel numbers are unique for merged probes
    for i =1:length(setF.device.Probe)
        ...
        PF2.curChSet=[PF2.curChSet,setF.device.Probe{i}.ChannelNumbers];
        PF2.curWvSet=[PF2.curWvSet,setF.device.Probe{i}.Wavelength];
        PF2.topoPlotInfo{i}=setF.device.Probe{i}.PlotStructure;
    end
...
if(~isempty(data))
    ...
    for prb=1:length(PF2.topoPlotInfo)
        ...
        optodes=curTopoInfo(:,1);
        subplotM=max(curTopoInfo(:,3));
        subplotN=max(curTopoInfo(:,2));

        for optIdx=1:length(optodes)

            subplot(subplotM,subplotN,sub2ind([subplotN,subplotM],curTopoInfo(optIdx,2),curTopoInfo(optIdx,3)));

            opt=optodes(optIdx);

            plotIdx=(ismember(data.channels,opt));
            %plotIdx2=(ismember(PF2.curConcSet,PF2.curConc));
            plotIdx=find(plotIdx);%.*plotIdx2);
            ...
            %*%xlim([PF2.view.startTime,PF2.view.endTime])
            title(sprintf('P%i: Opt%i',prb,opt));
            if(strcmp(PF2.GUIPF2.dpf_mode,'None'))
        ...
        if(preProcessed)
           suptitle(sprintf('Probe %i: Preprocessed Oxy',prb)); 
        else
            suptitle(sprintf('Probe %i: Processed Oxy',prb)); 
        end
    end
...
%%%%%% Plot for each probe
    for prb=1:length(PF2.topoPlotInfo)
        ...
        for optIdx=1:length(optodes)
            ...
            opt=optodes(optIdx);
            plotIdx=(ismember(PF2.curChSet,opt));
        ...
        if(preProcessed)
           suptitle(sprintf('Probe %i: Preprocessed Raw',prb)); 
        else
            suptitle(sprintf('Probe %i: Processed Raw',prb)); 
        end
    end 
end
