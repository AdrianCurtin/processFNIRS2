function [y] = pf2_SMAR_SplineReconstruct(x,fs,tauUp,tauLow,L,alpha)
...
%           deviation (MSD)
% tauUp:    Upper tau used in reconstrcution
% tauLow:	Lower Tau used in masking
...
for ch=1:numCh
	...
	% (1) Artefact detecion
	k = round(L/2);
	[A_Idx,low_Idx,s2_1,s2_2] = MADetection(curX,k,tauUp,tauLow,fs,alpha);
...
function [A_Idx,low_Idx,s2_1,s2_2] = MADetection(x,L,tauUp,tauLow,fs,alpha)
...
[s2] = calcLocalCV(x,L*2+1); s2_1 = s2;
...
if min(s2) > tauUp
    disp(['--->   MARA T threshold ' num2str(tauUp) ' is potentially too small, all data is seen as an artifact! std for data is between ' num2str(min(s2)) ' and ' num2str(max(s2))])
