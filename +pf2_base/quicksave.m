...
if(nargin<3)
    ...
    dataSerialized=hlp_serialize(var);
    nBytes=length(dataSerialized);
    warning('on','MATLAB:structOnObject');
    fprintf('Writing %i mb',nBytes/1e6)
    toc
    fwrite(fid,dataSerialized);
    fclose(fid);
    toc

    if(zipIt)
        filenameZip=strcat(filename,'.zip');
        fprintf('Zipping to %s...\n',filenameZip);
...
%                                (C) 2010 Tim Hutt
    ...
    elseif isjava(v)
        ...
        m = serialize_asbytes(v);
        %m = serialize_charstring(['<<hlp_serialize: ' class(v) ' unsupported>>']);
    else
        try
            m = serialize_object(v);
        catch
...
% char arrays
function m = serialize_charstring(v)
    if size(v,1) == 1
        % horizontal string: Type, Length, and Data
        m = [uint8(0); typecast(uint32(length(v)),'uint8').'; uint8(v(:))];
        nbytes = double(typecast(typecast(uint32(length(v)),'uint8').','uint32'));
        if(nbytes==135424)
            z=1;
        end
    elseif sum(size(v)) == 0
...
function m = serialize_cell(v)
    ...
    elseif isempty(v)
        ...
        if cellfun('isclass',v,'char') & size1 <= 1 %#ok<AND2>
            ...
            m = [uint8(36); serialize_charstring([v{:}]); serialize_numeric_simple(uint32(size2)); serialize_logical(size1(:)==0)];
...
function m = serialize_string(v)
    ...
    if isempty(v)
        ...
        % empty cell array
     %   m = [uint8(201); ndims(v); typecast(uint32(size(v)),'uint8').'];
    %else        
        % some non-scalar elements
        %dims = cellfun('ndims',v);
        ...
        %if cellfun('isclass',v,'char') & size1 <= 1 %#ok<AND2>
            % all horizontal strings or proper empty strings            
            m = [uint8(40); serialize_charstring([v{:}]); serialize_numeric_simple(uint32(size2)); serialize_logical(size1(:)==0)];
        %else
         %   error('Not a string')
       % end
    %end
...
% Object / class
function m = serialize_asbytes(v)
    try
        % try to use the saveobj method first to get the contents
        %conts = saveobj(v);

            % contents is still an object: turn into a struct now
            %%tempStruct=struct(conts);
            %%conts.Format="uuuu-MM-dd'T'HH:mm:ss.SSS'Z'";
            asBytes=getByteStreamFromArray(v);
            %%tempStruct.asString(ismissing(tempStruct.asString))='';
            conts = serialize_numeric_simple(asBytes);
    catch
        % saveobj failed for this object: turn into a struct
        conts = serialize_struct(struct(v));
    end
    % Tag, Class name and Contents
    m = [uint8(134); serialize_charstring(class(v)); conts];
end
...
function m = serialize_table(v)
    try
        ...
        % contents is still an object: turn into a struct now
        conts = struct(conts);
        fieldsToClear=['metaDim','rowDim','varDim'];
        conts = serialize_struct(conts);
    catch
        % saveobj failed for this object: turn into a struct
        conts = serialize_struct(struct(v));
    end
...
function m = serialize_object(v)
    try
        % try to use the saveobj method first to get the contents
        conts = saveobj(v);
        if isstruct(conts) || iscell(conts) || isnumeric(conts) || ischar(conts) || islogical(conts) || isa(conts,'function_handle') ||isstring(conts)||isdatetime(conts)||isduration(conts)
...
function m = serialize_handle(v)    
    ...
    switch rep.type
        case 'simple'
            ...
            m = [uint8(151); serialize_charstring(rep.function)];
        case 'anonymous'
            ...
            if isfield(tracking,'serialize_anonymous_fully') && tracking.serialize_anonymous_fully
                ...
                % Tag and Code
                m = [uint8(152); serialize_charstring(char(v))];
                ...
                if ~isempty(rep.workspace)
                    m = [uint8(152); serialize_charstring(char(v)); serialize_struct(rep.workspace{1})];
                else
                    m = [uint8(152); serialize_charstring(char(v)); serialize_struct(struct())];
                end
            end
        ...
        otherwise
            ...
            m = serialize_charstring(['<<hlp_serialize: function handle of type ' rep.type ' unsupported>>']);
    end
end
