...
if(nargin<3)
    ...
    filenameTempDir='';
    tic
    if(unzipIt||contains(filename,'.zip'))
    ...
    nBytes=length(dataRead);
    fprintf('Loaded %i mb\n',nBytes/1e6)
     fclose(fid);
    toc
     if(unzipIt||contains(filenameZip,'.zip'))
         fprintf('Removing temp file & directory: %s...\n',filename);
        delete(filename);
        rmdir(filenameTempDir);
     end
     fprintf('Deserializing file...\n');
    dataDeserialized=hlp_deserialize(dataRead);
    fprintf('Finished!\n');
...
switch kind
    ...
    case 39 % boolean flags
        ...
        for k=1:numel(v)
            v{k} = content(k); end
    case 40 % cell array from string
        [chars,pos] = deserialize_string(m,pos);
        [lengths,pos] = deserialize_numeric_simple(m,pos);
        [empty,pos] = deserialize_logical(m,pos);
        v = cell(size(lengths));
...
asBytesConversions = ['duration','datetime'];

if(ismember(cls,asBytesConversions))
    v=getArrayFromByteStream(conts);
else
    % construct object
    try
