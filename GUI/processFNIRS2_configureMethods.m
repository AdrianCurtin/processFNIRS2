    ...
    if(isfield(PF2.currentMethod.F{curFunction},'f')&&ismember(PF2.currentMethod.F{curFunction}.f,PF2.myFunctions.cfg.Sections))
        ...
        try
            ...
            if(~ismember(curArgument,reservedArgs))
                ...
                elseif(strcmp(curArgument,'fNIRstruct'))
                    set(handles.text_inputDescription,'String',sprintf('Full fNIR data structure')); 
                end
            end
...
if(iscell(f2add))
   for i=1:length(f2add)
      ...
      PF2.currentMethod.F{length(PF2.currentMethod.F)}.output=PF2.myFunctions.cfg.(f2add{i}).Output;
      ...
      for a=1:length(PF2.currentMethod.F{length(PF2.currentMethod.F)}.args)
          ...
          elseif(strcmp('fNIRstruct',PF2.myFunctions.cfg.(f2add{i}).Arguments{a}))
              PF2.currentMethod.F{length(PF2.currentMethod.F)}.argvals{a}='Full fNIR data structure'; 
          else
              PF2.currentMethod.F{length(PF2.currentMethod.F)}.argvals{a}=[];
          end
      end
