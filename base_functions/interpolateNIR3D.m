function interpolateNIR3D(varargin) %(fNIRarr,minVal,maxVal,interpolationType,transparent,brainStyle,titleTxt,fontSize,clr,showChText)
...
if(interpolationType==1) %%Should remove everything not inbetween optodes 
...
if(interpolationType~=2)     %259 from top to first row-mid, 144 from left to first row mid %233 from right to last, %408 from bottom
    ...
    for (j=1:length(yChLoc))
        ...
        if(showChannelLabels)
            if(~brainStyle)
                ...
                chT=text(xChLoc(i)-cLen*8,yChLoc(j),sprintf('%i',chNum),'FontWeight','Bold');
                chT.Color=[1,1,1];
            else
                %plot(xChLoc(i), yChLoc(j), '.k', 'MarkerSize',42);
                chT=text(xChLoc(i)-cLen*6,yChLoc(j),sprintf('%i',chNum),'FontWeight','Bold');
                chT.Color=[0,0,0];
            end
        else
