function fNIR=getTakizawaRejection(fNIR,applyMask,strictCriteria)
     ...
     if(nargin<2)
         applyMask=false; % merges mask with fchMask if enabled
     end
    ...
    if(applyMask)
        if(strictCriteria)
            fNIR.fchMask=fNIR.fchMask&takizawa.fchMask_strict;
        else
            fNIR.fchMask=fNIR.fchMask&takizawa.fchMask;
        end
    end
