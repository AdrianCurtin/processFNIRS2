    ...
    if(~isempty(devinfo))
        ...
        elseif(iscell(ch_index))
            for roi_ind=1:length(ch_index)
                ...
                if(roi_ind==1)
                    roi_out.ROI.info=table(ch_index(roi_ind),{'oxy'},'RowNames',roi_names(roi_ind),'VariableNames',{'Channels','Type'});
                else
                    roi_out.ROI.info=[roi_out.ROI.info;table(ch_index(roi_ind),{'oxy'},'RowNames',roi_names(roi_ind),'VariableNames',{'Channels','Type'})];
                end
            end
        end
