...
%
	subplot(221); semilogy(reverse_wl(cumsum(reverse_wl(wcBlocks.^2))));
	title('Blocks; First-m')
	xlabel('m')
	ylabel('d_m')
	axis([0 512 1 10^5])
%
	subplot(222); semilogy(reverse_wl(cumsum(reverse_wl(wcBumps.^2))));
	title('Bumps; First-m')
	xlabel('m')
	ylabel('d_m')
	axis([0 512 1 10^5])
%
	subplot(223); semilogy(reverse_wl(cumsum(reverse_wl(wcHeaviSine.^2))));
	title('HeaviSine; First-m')
	xlabel('m')
	ylabel('d_m')
	axis([0 512 1 10^5])
%
	subplot(224); semilogy(reverse_wl(cumsum(reverse_wl(wcDoppler.^2))));
