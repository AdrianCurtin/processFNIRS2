...
%
	subplot(221); semilogy(reverse_wl(cumsum(sort(wcBlocks.^2))));
	title('Blocks; Best-m')
	xlabel('m')
	ylabel('c_m')
	axis([0 512 1 10^5])
%
	subplot(222); semilogy(reverse_wl(cumsum(sort(wcBumps.^2))));
	title('Bumps; Best-m')
	xlabel('m')
	ylabel('c_m')
	axis([0 512 1 10^5])
%
	subplot(223); semilogy(reverse_wl(cumsum(sort(wcHeaviSine.^2))));
	title('HeaviSine; Best-m')
	xlabel('m')
	ylabel('c_m')
	axis([0 512 1 10^5])
%
	subplot(224); semilogy(reverse_wl(cumsum(sort(wcDoppler.^2))));
