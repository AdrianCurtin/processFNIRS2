...
save('tests/golden/candidates/processFNIRS2_sampleNIR_x2_lpf_smar.mat', '-struct', 'golden');
```
...
save('tests/golden/candidates/pf2_SMAR_sampleNIR.mat', '-struct', 'golden');
```
...
classdef ProcessingGoldenTest < matlab.unittest.TestCase
    ...
    properties (TestParameter)
        goldenFile = getGoldenFiles('tests/golden/processFNIRS2');
    end
...
function promoteCandidate(candidateName, targetDir)
    ...
    candidatePath = fullfile('tests', 'golden', 'candidates', candidateName);
    targetPath = fullfile('tests', 'golden', targetDir, candidateName);
    ...
    if ~isfolder(fullfile('tests', 'golden', targetDir))
        mkdir(fullfile('tests', 'golden', targetDir));
    end
...
3. **Review the differences**:
   ```matlab
   compareGolden('tests/golden/processFNIRS2/file.mat', 'tests/golden/candidates/file.mat');
   ```
