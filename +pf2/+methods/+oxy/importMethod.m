...
% IMPORTMETHOD Import an oxy processing method from a JSON file
%
% Loads an oxy processing method from a portable JSON file exported by
% exportMethod. Creates the method in the current installation.
%
% Syntax:
%   pf2.methods.oxy.importMethod(filePath)
%   pf2.methods.oxy.importMethod(filePath, 'Replace', true)
%
% Inputs:
...
%
% Example:
%   pf2.methods.oxy.importMethod('/path/to/method.json');
%   pf2.methods.oxy.importMethod('/path/to/method.json', 'Replace', true);
%
% See also: pf2.methods.oxy.exportMethod, pf2.methods.raw.importMethod,
...
% Validate type
if isfield(jsonStruct, 'type') && ~strcmp(jsonStruct.type, 'oxy')
    error('pf2:TypeMismatch', ...
        'Method type is ''%s'', expected ''oxy''. Use pf2.methods.raw.importMethod for raw methods.', ...
        jsonStruct.type);
end
...
pf2.methods.oxy.create(methodName, functions, 'Replace', replaceExisting);

fprintf('Imported oxy method ''%s'' from %s (%d functions)\n', ...
