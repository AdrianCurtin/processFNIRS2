...
close(fig);
fprintf('  Saved matrix + chord figures\n');
...
close(fig);
fprintf('  Saved condition-specific matrices\n');
...
close(fig);
fprintf('  Saved ROI matrix + chord\n');
...
close(fig);
fprintf('  Saved group beta-series matrix\n');
...
close(fig);
fprintf('  Saved group PPI bar chart\n');
...
d_out = dir(fullfile(outDir, 'step*'));
for i = 1:length(d_out)
    fprintf('  %s (%.1f KB)\n', d_out(i).name, d_out(i).bytes/1024);
end
